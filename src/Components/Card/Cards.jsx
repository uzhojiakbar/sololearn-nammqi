    ...
    <div>
      <div className="bg-[#F2F5F7] " >
        ...
        <div className=" py-12 px-10">
          <div className="flex flex-col-reverse  bg-white rounded-sm sm:grid sm:grid-cols-2 md:grid-cols-2 sm:gap-12 lg:relative">
            {card2?.map((item) => (
              <div key={item.id} className=" p-6 max-sm:text-center">
                <h1 className="lg:text-[40px] md:text-xl text-xl font-bold text-[#2D3846]">{item.name}</h1>
                <p className="text-[#6B7F99] lg:text-[20px] mt-2">{item.desc}</p>
                <h1 className="text-[24px] text-[#222222] font-bold">{item.qrname}</h1>
                <div className="mt-4 flex items-center gap-6">
                  <img src={item.childImg1} className=" w-[140px] md:w-[136px] sm:w-[100px]" alt="" />
                  <div className="">
                    <img src={item.appstoe} className="sm:w-[146px] " alt="apple" />
                    <img src={item.google_play} className="sm:w-[146px]" alt="google" />
                  </div>
                </div>
              </div>
            ))}
