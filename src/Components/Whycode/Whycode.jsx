    ...
    <div className="bg-[#eaf0f3] py-24 flex flex-col items-center gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-3 gap-8 w-full px-8 mt-28 max-sm:space-y-72 lg:mt-52">
        {WhyData?.map((item) => (
          <div
            key={item.id}
            className="relative  bg-white px-5 py-8 pt-20 text-center rounded-sm max-sm:pt-12 max-sm:mt-48 "
          >
            <img
              src={item.image}
              alt="logoimg"
              className="absolute lg:translate-y-[-110%] sm:translate-y-[-120%] left-0 max-sm:translate-y-[-128%] max-sm:left-[50%] max-sm:translate-x-[-50%]"
            />
